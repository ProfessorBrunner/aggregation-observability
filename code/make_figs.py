...
plt.rcParams.update({'font.size':10,'axes.labelsize':9,'legend.fontsize':7.5,'figure.dpi':150,'font.family':'serif'})
W=4.4  # single PRE column width, inches
...
fig.tight_layout(); fig.savefig('../figures/fig_g2.pdf',bbox_inches='tight',pad_inches=0.03); plt.close(fig)
...
ax.legend(frameon=False,loc='upper center',bbox_to_anchor=(0.5,-0.22),ncol=2); fig.set_size_inches(W,3.9); fig.tight_layout(); fig.savefig('../figures/fig_undershoot.pdf',bbox_inches='tight',pad_inches=0.03); plt.close(fig)
...
ax.axvline(1.3858,color='C0',lw=0.8,ls='--'); ax.text(1.40,1.04,'convolution prediction',fontsize=8,color='C0')
ax.set_xlim(0.8,2.1); ax.set_ylim(-0.03,1.12); ax.set_xlabel(r'$\mathrm{sd}(\eta)/\Delta$'); ax.set_ylabel('fraction of draws\nreaching $m=-0.5$')
ax.legend(frameon=False,loc='center right'); fig.tight_layout(); fig.savefig('../figures/fig_definedness.pdf',bbox_inches='tight',pad_inches=0.03); plt.close(fig)
...
ax.axvline(766,color='0.5',lw=0.7); ax.axhline(0.0225,color='C3',lw=0.7,ls=':'); ax.set_xlabel(r'sample size $N$'); ax.set_ylabel(r'detectable $|C_q|$'); ax.legend(frameon=False,loc='upper center',bbox_to_anchor=(0.5,-0.22),ncol=1,fontsize=6.5); fig.set_size_inches(W,4.2)
fig.tight_layout(); fig.savefig('../figures/fig_detect.pdf',bbox_inches='tight',pad_inches=0.03); plt.close(fig)

# ---------- Fig 5: Lambda across datasets (abortion pair from published table) ----------
...
ax.text(1,3.5e-4,r'$\Lambda=0$ plotted at $10^{-4}$',fontsize=6.5,color='0.4')
ax.set_xlabel('dataset'); ax.set_ylabel(r'$\Lambda=2(\ell_{\rm QQ}-\ell_{\rm P})$')
fig.tight_layout(); fig.savefig('../figures/fig_lambda.pdf',bbox_inches='tight',pad_inches=0.03); plt.close(fig)
