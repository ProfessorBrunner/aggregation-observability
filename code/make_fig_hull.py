...
plt.rcParams.update({'font.size':10,'axes.labelsize':9,'legend.fontsize':7.5,'font.family':'serif'})
...
ax.add_patch(plt.Circle((0,0),1,fill=True,color='C0',alpha=0.12,lw=0))
ax.add_patch(plt.Circle((0,0),1,fill=False,color='C0',lw=1))
...
ax.text(0,1.07,'projective bound',ha='center',fontsize=7.5,color='C0')
ax.set_xlim(-2.8,2.8); ax.set_ylim(-1.5,2.1); ax.set_aspect('equal')
ax.set_xlabel(r'$x=(\bar q-\bar r)/\sqrt{1-\bar a}$'); ax.set_ylabel(r'$y=(\bar q+\bar r-1)/\sqrt{\bar a}$')
ax.legend(frameon=False,loc='upper left',fontsize=6.8,handletextpad=0.3)
fig.tight_layout(); fig.savefig('../figures/fig_hull.pdf',bbox_inches='tight',pad_inches=0.03)
